...
const ICONS = {
    ...
    dots: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="5" r="1.5"></circle><circle cx="12" cy="12" r="1.5"></circle><circle cx="12" cy="19" r="1.5"></circle></svg>`
};
...
function showToolbarMenu(x, y) {
    ...
    moreDropdown.style.cssText = `
        ...
        box-shadow: 0 8px 24px rgba(0,0,0,0.12); min-width: 140px; z-index: 9999;
        padding: 6px 0; flex-direction: column; overflow: hidden;
    `;

    const createDropdownItem = (icon, text, onClick) => {
        const item = document.createElement('div');
        item.innerHTML = `<span style="display:flex; align-items: center; justify-content: center; margin-right: 10px; color: #555;">${icon}</span> <span style="font-weight: 400;">${text}</span>`;
        item.style.cssText = `padding: 8px 16px; font-size: 13px; cursor: pointer; display: flex; align-items: center; color: #333; transition: background 0.15s;`;
...
function showAIMenu(x, y) {
    ...
    popupUI.style.cssText = `
        ...
        border-radius: 8px; z-index: 2147483647;
        font-family: system-ui, -apple-system, sans-serif; font-size: 13px;
        color: #333; width: max-content; min-width: 200px; 
        padding: 4px;
    `;
...
// Прямой вызов без лишних обработчиков
