"use strict";
let currentSelection = { text: "", context: "", range: null, activeElement: null, start: null, end: null, isInput: false };
let popupUI = null;
...
document.addEventListener('mousemove', (e) => {
    ...
    if (newY < 0)
        newY = 0;
    if (newX + popupUI.offsetWidth > window.innerWidth)
        newX = window.innerWidth - popupUI.offsetWidth;
    if (newY + popupUI.offsetHeight > window.innerHeight)
        newY = window.innerHeight - popupUI.offsetHeight;
...
document.addEventListener('mouseup', (e) => {
    ...
    if (isDragging && popupUI) {
        isDragging = false;
        const header = popupUI.querySelector('.gemini-header');
        if (header)
...
chrome.storage.local.get(['selectedTheme', 'searchEngine'], (res) => {
    if (res.selectedTheme)
        currentTheme = res.selectedTheme;
    if (res.searchEngine)
        currentSearchEngine = res.searchEngine;
});
...
function getPopupContainer() {
    ...
    if (activeEl && activeEl.closest('dialog')) {
        ...
        if (sel && sel.rangeCount > 0) {
            ...
            if (node && node.closest('dialog'))
                container = node.closest('dialog');
        }
    }
    return container;
}
...
function saveSelectionState(fallbackText) {
    const activeEl = document.activeElement;
    const sel = window.getSelection();
    ...
    if (activeEl && (activeEl.tagName === 'TEXTAREA' || activeEl.tagName === 'INPUT')) {
        ...
        if (!currentSelection.text && fallbackText)
            currentSelection.text = fallbackText;
        ...
        if (blockText && blockText.length > 2000) {
            ...
            if (idx !== -1)
                currentSelection.context = blockText.substring(Math.max(0, idx - 1000), Math.min(blockText.length, idx + currentSelection.text.length + 1000));
            else
                currentSelection.context = currentSelection.text;
        }
        else {
        ...
        streamPort.onMessage.addListener((response) => {
            if (response.status === "chunk") {
                ...
                if (safeHtml.includes('**'))
                    safeHtml = safeHtml.replace(/\*\*([^*]*)$/, '<mark>$1</mark>');
...
function adjustPopupPosition() {
    if (!popupUI || isManuallyPositioned)
        return;
    const rect = popupUI.getBoundingClientRect();
